  ...
  return (
    <Link href="/" aria-label="Go to homepage">
      ...
      >
        ...
        >
          {/* Background fill that fades in after stroke draws */}
          <motion.path
            d={pathD}
            className={className || "fill-markus-red"}
            variants={{
              hidden: { fillOpacity: 0 },
              visible: { fillOpacity: 1 }
            }}
            transition={{ delay: 1.5, duration: 0.5 }}
          />
          {/* Stroke that draws in */}
          <motion.path
            d={pathD}
            fill="none"
            stroke="currentColor"
            strokeWidth={2}
            className={className ? className.replace('fill-', 'stroke-') : "stroke-markus-red"}
            variants={{
              hidden: { pathLength: 0 },
